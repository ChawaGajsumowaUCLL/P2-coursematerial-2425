...
def sum_movie_duration_in_90s(movies, start_year, end_year):
    total_duration = 0
    for movie in movies:
...
def find_string_starting_with_a(strings, letter):
    for string in strings:
        if len(string) != 0 and string[0].lower() == letter:
            return string
    return None

def find_number_greater_than_100(numbers, threshold):
